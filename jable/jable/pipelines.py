...
def encode_chinese_path(path: str) -> str:
    """
    对中文路径做URL编码（核心解决ASCII编码问题）
    示例：/影视/测试.mp4 → /%E5%BD%B1%E8%A7%86/%E6%B5%8B%E8%AF%95.mp4
    """
    # 分割路径，避免编码/和.等符号，只编码中文部分
    path_parts = path.split("/")
    encoded_parts = []
    for part in path_parts:
        if part:  # 跳过空字符串（路径开头的/分割后会有空值）
            encoded_part = urllib.parse.quote(part, encoding="utf-8")  # 强制UTF-8编码
    ...
    # 处理末尾的/（可选）
    if path.endswith("/") and not encoded_path.endswith("/"):
        encoded_path += "/"
    return encoded_path
...
async def upload_video_by_official_api_async(
    ...
    headers = {
        ...
        "path": encode_chinese_path(target_path),
    ...
    try:
        # 异步客户端 + 异步文件读取（核心）
        async with httpx.AsyncClient(timeout=300) as client:  # 大文件超时设5分钟
            ...
            with open(local_file_path, "rb") as f:
                ...
                if result.get("code") == 200 and result.get("message") == "success":
                    ...
                    raise Exception(f"上传失败：{result.get('message', '官方未返回原因')}")
    except Exception as e:
...
class UploadPipeLines:
    async def process_item(self, item, spider):
        ...
        result = await upload_video_by_official_api_async(OPENLIST_URL, token, LOCAL_VIDEO_PATH, OPENLIST_TARGET_PATH)
        print("上传结果", result)
        print(f"结束上传了 {url}")
        endtime = time.perf_counter()
